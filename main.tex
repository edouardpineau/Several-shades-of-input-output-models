...
\author{
  Edouard Pineau \\
  \texttt{Natixis}\\
  \texttt{pineau.edouard@gmail.com}
}
...
\paragraph{Objective of the paper} We identified gaps in the literature regarding the systematic comparison of IO model variants and the implications of different stressing assumptions for both theoretical consistency and practical implementation. 
In particular, in the paper, we clarify theoretically the role of shock origin and propagation. We provide a unified and rigorous comparison of outcomes when shocks are applied to different key variables: final demand, gross value added, intermediate consumption, and total output. By explicitly contrasting these stressing routes within a common framework, we show how apparently similar stress tests can lead to materially different sectoral and geosectoral results, and we explain the economic interpretations and limitations associated with each choice. 
Moreover, we examine in detail the interaction between price and production equations, an aspect that is often treated piecemeal in applied stress testing work. Our discussion clarifies how price shocks (e.g. carbon prices or biodiversity-related cost changes) propagate through IO systems, under different modeling choices, and how these price dynamics feed back into production volumes and sectoral outputs in geosectoral stress tests.
We formalize pass-through mechanisms and geosectoral feedbacks in these different views of IO framework. We investigate how binary pass-through assumptions relate to the absence of geosectoral feedbacks in standard IO implementations. We show how continuous pass-through rates can be consistently embedded into the Leontief framework under all stressing assumptions. 
Beyond these conceptual contributions, the paper aims to be applied. We propose an implementation and examples in the concrete context of environmental stress tests for the financial institutions. 
...
\begin{equation}
    \Delta v = \underbrace{\mathcal{I - A}^T \Delta p(\phi)}_{\text{Value chain inflation}} - \underbrace{\left(I - \Phi\right) \Delta c}_{\text{Direct costs}} = \left[ \left(I - A^T\right) \mathcal{L}(\phi) - (I - \Phi) \right] \Delta c
...
When not available at sectoral level, we infer PED from the PTR. In \cite{weyl2013pass}, authors developed a general formula that relates PED to PTR, given in Eq. \eqref{eq:ptr_elasticities}
...
is a \textit{conduct parameter} (is typically used to measure competitiveness, i.e., the extent to which firms in a market behave competitively, cooperatively, or monopolistically.), and $\epsilon^{y,p}$, $\epsilon^{x,p}$, $\epsilon^{\theta,x}$ and $\epsilon^{ms,p}$ are respectively the elasticities of demand (w.r.t. price $p$), production (w.r.t. price $p$), conduct (w.r.t. production $x$) and marginal surplus (w.r.t. demand $p$). All details to obtain this formula are supplied in \cite{weyl2013pass,rbb2014cost}. In particular, by definition of the conduct parameter, they precise that $\theta$ takes value depending on the market structure: $\theta=1$ for a monopoly, $\theta=0$ for perfect competition, $\theta \in [0,1]$ for oligopolistic situations (e.g., $\theta=1/N$ in a Cournot competition between $N$ firms). 
...
In this section, we display results of environmental-related stress testing using the different shades of IO modeling presented above. We use FIGARO IO database, aggregated at NACE1 level in each of the 44 countries, in 2024 (last year available when the paper's experiments were generated). The other IODs are presented in Appendix \ref{appendix:io_databases}.  We note that the theoretical framework and results presented in the paper are agnostic to the choice of the database or the year. Yet, in practice, changing the data may change the absolute values of the results, note the illustration of the paper's results. Hence, the choice of the database is an assumption to monitor for pratical usage. 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.49\linewidth]{deltap_by_ptr_comparisons.png}
    \includegraphics[width=0.49\linewidth]{deltap_by_ptr_pairwise_diff.png}
...
\noindent Following the \href{https://link.springer.com/journal/40008/submission-guidelines/research-article}{giudelines of the Journal of Economic Structures}, author declares:

\paragraph{Availability of data and materials} The data to reproduce the experiments of the paper are IO tables. The hyperlinks are given in Appendix \ref{appendix:io_databases}. As explained at the beginning of the Section \ref{sec:use_cases}: \textit{We use FIGARO IO database, aggregated at NACE1 level in each of the 44 countries, in 2024 (last year available when the paper's experiments were generated). [...] We note that the theoretical framework and results presented in the paper are agnostic to the choice of the database or the year. Yet, in practice, changing the data may change the absolute values of the results, note the illustration of the paper's results. Hence, the choice of the database is an assumption to monitor for pratical usage. }

\vspace{0.1cm}

To obtain the databases, please follow the \href{https://circabc.europa.eu/ui/group/cec66924-a924-4f91-a0ef-600a0531e3ba/library/e6896a70-5dbf-479c-8852-94b5dd1e3952?p=1&n=10&sort=modified_DESC}{link}. You then download the csv file for the year of interest. The file contains all the information needed for the IO modeling. 

\vspace{0.1cm}

The full code and data that support the findings of this study are available but restrictions apply to the availability of these. Code and data are however available from the authors upon reasonable request.
 

\paragraph{Competing interests} No financial and non-financial competing interests are involved here.

\paragraph{Funding} No specific funding to declare for this paper. It has been written on free time, but using my employer's computer (I don't have personnel computer). 

\paragraph{Authors' contributions} EP fully contributed since he is the only author. 

\paragraph{Acknowledgements} Not applicable : no other contributors. 

\paragraph{Authors' information} With a background in applied mathematics studied at Université Paris-Dauphine and ENS Cachan, Edouard is specialized in the integration of environmental factors into economic and financial risks. For four years after defending his PhD at Institut Polytechnique de Paris, he worked with banks and investors to help them model and quantify their exposure to physical and transitional climate risks and ecosystem degradation. 

\paragraph{Ethics approval and consent to participate} Not applicable. 

\paragraph{Consent for publication} Author consents to publish the paper. No other human involved. 
...
\href{https://worldmrio.com/}{EORA} & 190 countries & 1990-Present & 26-500+ \\ \hline
\href{https://exiobase.eu/}{EXIOBASE} & 44 countries + 5 RoW & 1995-Present & 163 \\ \hline
\href{https://ec.europa.eu/eurostat/web/esa-supply-use-input-tables/database}{FIGARO} & 45 countries + 1 RoW & 2010-Present & 64 \\ \hline
\href{https://www.rug.nl/ggdc/valuechain/wiod/}{WIOD} & 43 countries + 1 RoW & 2000-2014 & 56  \\ \hline
\end{tabular}
